...
The training data consists of `r paste(dim(training), collapse = ' rows and ')` columns. However, of these columns many have upto `r round(max(colSums(is.na(training)))*100/nrow(training),1)`% missing observations. The columns without any missing observations are selected for the future analysis. Furthermore, the columns user_name and classe are categorical variables and are converted into factors to reflect this. 
